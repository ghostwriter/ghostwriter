# ghostwriter

#### ✊🏿🖤✊🏻 `#BlackLivesMatter` | 🇵🇸 `#FreePalestine` | 🇮🇷 `#StandWithIran` | 🇺🇦 `#StandWithUkraine`

<img alt="Created by a Human, not by AI" src="./templates/badge.svg">
  
All of my open-source projects are open to **contributions**, **feedback**, and **suggestions**.

If you find a bug, have a feature request, or want to contribute:
- **Open an issue**
- **Submit a pull request**

Managing hundreds of projects as a hobby is an incredible workload, and every contribution helps!

---

If you want to support me financially, [become a sponsor](https://github.com/sponsors/ghostwriter).

---

> - **you** are your **only** competition.
> - if you don't like something, **change it**!
> - knowledge **should** be **freely** accessible.
> - **privacy** is a **feature**, **not** a **product**.
> - **security** is a **process**, **not** a **feature** or a **product**.
> - **performance** is a **process**, **not** a **feature** or a **product**.
> - **contributions** must be **attributed** and or **compensated**.
> - have the confidence to make **impactful decisions** that are **selfish**, **your well-being is more important**!
> - **good** products **fail** because of **bad marketing**, **awful** products **succeed** with **great marketing**.
> - **AI** processes **input** (your questions and data) and generates **output** based on **patterns** and **algorithms**.

---

<div align="center">
    <img alt="ghostwriter's GitHub streak" height="150px" src="https://streak-stats.demolab.com/?user=ghostwriter">
</div>

#### Recent Releases
| State | Description |
| --- | --- |
| (TAGGED) | Release [0.1.2](https://github.com/ghostwriter/testify/releases/tag/0.1.2) on [ghostwriter/testify](https://github.com/ghostwriter/testify) (today)|
| (TAGGED) | Release [3.0.2](https://github.com/ghostwriter/clock/releases/tag/3.0.2) on [ghostwriter/clock](https://github.com/ghostwriter/clock) (today)|
| (TAGGED) | Release [0.1.0](https://github.com/ghostwriter/libraries-io/releases/tag/0.1.0) on [ghostwriter/libraries-io](https://github.com/ghostwriter/libraries-io) (today)|
| (TAGGED) | Release [7.0.2](https://github.com/ghostwriter/container/releases/tag/7.0.2) on [ghostwriter/container](https://github.com/ghostwriter/container) (today)|
| (TAGGED) | Release [0.2.0](https://github.com/ghostwriter/http/releases/tag/0.2.0) on [ghostwriter/http](https://github.com/ghostwriter/http) (4 days ago)|

#### Recent Contributions

> [!CAUTION]
> Repositories with "[WIP]" indicate that the project is currently under development, and it may not be ready for public consumption.

| Repo | Description |
| --- | --- |
| ⭐️ (0) [`ghostwriter/console`](https://github.com/ghostwriter/console) | console application skeleton built on top of symfony/console |
| ⭐️ (0) [`ghostwriter/cli-parser`](https://github.com/ghostwriter/cli-parser) | Command line arguments/options parser. |
| ⭐️ (1) [`ghostwriter/container`](https://github.com/ghostwriter/container) | Provides an extensible Dependency Injection Service Container for Automated Object Composition, Interception, and Lifetime Management. |
| ⭐️ (0) [`ghostwriter/http`](https://github.com/ghostwriter/http) | HTTP Client and Server abstraction for PHP. |
| ⭐️ (2) [`ghostwriter/event-dispatcher`](https://github.com/ghostwriter/event-dispatcher) | Event Dispatcher implementation. |
| ⭐️ (0) [`ghostwriter/draft`](https://github.com/ghostwriter/draft) | [WIP] A code generation tool for Laravel developers |
| ⭐️ (0) [`ghostwriter/compliance`](https://github.com/ghostwriter/compliance) | [WIP] Compliance - CI/CD &amp; QA Test Automation for PHP via GitHub Actions |
| ⭐️ (0) [`BMHVillage/therapists`](https://github.com/BMHVillage/therapists) | Collection of therapists in Nashville, TN who are Black/African American, as well as therapists serving Black/African American communities. This publicly available information is gathered to support the promotion of healing for ourselves and others. #BlackLivesMatter |
| ⭐️ (0) [`nix-php/nixify`](https://github.com/nix-php/nixify) | Nix tool for PHP |
| ⭐️ (2) [`ghostwriter/wip`](https://github.com/ghostwriter/wip) | [WIP] Here be dragons |
| ⭐️ (0) [`ghostwriter/filesystem`](https://github.com/ghostwriter/filesystem) | Filesystem implementation for PHP |
| ⭐️ (0) [`ghostwriter/config`](https://github.com/ghostwriter/config) | Provides an object that maps configuration keys to values. |
| ⭐️ (1) [`ghostwriter/handrail`](https://github.com/ghostwriter/handrail) | Safeguard PHP functions from redeclaration conflicts |
| ⭐️ (0) [`ghostwriter/psr-phpunit-assertions`](https://github.com/ghostwriter/psr-phpunit-assertions) | [WIP] PHPUnit assertions for PSR-compliant components, implemented as reusable traits |
| ⭐️ (1) [`ghostwriter/testify`](https://github.com/ghostwriter/testify) | Automated test Generation for PHP code. |
| ⭐️ (0) [`ghostwriter/libraries-io`](https://github.com/ghostwriter/libraries-io) | A libraries.io API Client for PHP |
| ⭐️ (0) [`ghostwriter/advent-of-code`](https://github.com/ghostwriter/advent-of-code) | Advent of Code |
| ⭐️ (0) [`ghostwriter/phpt`](https://github.com/ghostwriter/phpt) | [WIP] Testing Framework for PHP |
| ⭐️ (1) [`ghostwriter/shell`](https://github.com/ghostwriter/shell) | Execute commands and external programs |
| ⭐️ (3) [`ghostwriter/option`](https://github.com/ghostwriter/option) | Provides an Option type implementation for PHP |
| ⭐️ (0) [`ghostwriter/uuid`](https://github.com/ghostwriter/uuid) | Version 7 UUIDs using a Unix timestamp for PHP |
| ⭐️ (2) [`ghostwriter/syntax`](https://github.com/ghostwriter/syntax) | [WIP] Lexical Analysis library for PHP |
| ⭐️ (0) [`ghostwriter/phar-tools`](https://github.com/ghostwriter/phar-tools) | [WIP] Centralized PHAR toolchain binaries for ghostwriter projects |
| ⭐️ (3) [`ghostwriter/psalm-sandbox`](https://github.com/ghostwriter/psalm-sandbox) | Provides a framework for testing Psalm plugins |
| ⭐️ (2) [`ghostwriter/result`](https://github.com/ghostwriter/result) | Provides a Result type implementation for PHP |
